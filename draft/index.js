...
function after({dirname, mode, m2units: {units, dir = "m2units/"}}) {
    ...
    return function (app) {
        ...
        if(fs__WEBPACK_IMPORTED_MODULE_1___default.a.existsSync(path.resolve(__dirname, "../../../m2units.json"))) {

            console.log("m2units file exists - modules can be overwritten");

            let m2units = eval("require")("../../../m2units.json");
            m2units = Object.keys(m2units).map( key => ({ name: key, npm: m2units[key] }) );
            units.push(...m2units);
        }

        let { m2units = [] } = eval("require")("../../../package");
        ...
        app.get(`/${dir}*`, function(req, res) {
            ...
            let { m2units = {} } = eval("require")(`../../../node_modules/${name}/package`);
